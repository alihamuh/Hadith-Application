...
public class BookMark extends AppCompatActivity {
    ...
    ArrayList<Integer> bookmarkedHadithNo =new ArrayList<>();
    ArrayList<Integer> bookmarkedChapterNo = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //ArrayList<Integer> bookmarkedHadithNo=new ArrayList<>();
        //ArrayList<String> bookmarkedChapterNo= new ArrayList<>();
        //ArrayList<String> SuraName= new ArrayList<>();

        for(int index =1; index<=settings.getInt("bookmark_no",0);index++){

            bookmarkedHadithNo.add(index-1,settings.getInt("HadithBookmark_"+index,0));
            bookmarkedChapterNo.add(index-1,settings.getInt("ChapterBookmark_"+index,0));
        ...
        adapter = new BookMarkArrayAdapter(this, bookmarkedHadithNo,bookmarkedChapterNo);
                ...
                public void onClick(View v) {
                    ...
                    for(int i=0;i<Myapp2.getChecks().size();i++){
                        ...
                        if(Myapp2.getChecks().get(i)==1){
                            ...
                            bookmarkedHadithNo.remove(i);
                            bookmarkedChapterNo.remove(i);
                            //similarly remove other items from the list from that particular postion
                            SharedPreferences settings =PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
                            SharedPreferences.Editor editor =settings.edit();

                            editor.remove("HadithBookmark_"+index);
                            editor.remove("ChapterBookmark_"+index);
